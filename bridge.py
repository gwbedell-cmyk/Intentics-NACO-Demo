...
class MGEPlusBridge:
    ...
    # Add slight signal variation from embedding (safe)
    emb = self.get_embedding(text)
    emb_np = emb.detach().cpu().numpy()
    variation = np.abs(emb_np[:4])
    variation = variation / (np.sum(variation) + 1e-12)

    # Blend probe + variation
    s = 0.7 * s + 0.3 * variation
    ...
    def compute_gap(self, founder_text, investor_text, s_f, s_i):
        ...
        if "timeline" in text or "milestone" in text:
            idx = 3
        elif "trust" in text or "team" in text:
            idx = 0
        elif "impact" in text or "ethics" in text:
            idx = 1
        elif "vision" in text or "strategy" in text:
            idx = 2
    ...
    def get_adjustments(self, gap_idx):
        return (
            ...
            "Define the minimum proof needed to feel confident moving forward."
        )
    ...
    def run_dual_analysis(self, founder_text, investor_text):
        ...
        f_adj, i_adj = self.get_adjustments(gap_idx)

        return {
            "score": result["score"],
            "gap": gap_text,
            "founder_adjustment": f_adj,
            "investor_adjustment": i_adj,
            "improved_score": min(result["score"] + 15, 95)
        }
