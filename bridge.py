...
class MGEPlusBridge:
    ...
    def compute_gap(self, founder_text, investor_text, s_f, s_i):
        ...
        text = (founder_text + " " + investor_text).lower()

        if any(w in text for w in ["timeline", "milestone", "ship", "execute", "deliver"]):
            idx = 3
        elif any(w in text for w in ["trust", "team", "reliable", "integrity"]):
            idx = 0
        elif any(w in text for w in ["impact", "fair", "ethics", "patients", "people"]):
            idx = 1
        elif any(w in text for w in ["vision", "strategy", "thinking", "approach"]):
            idx = 2
